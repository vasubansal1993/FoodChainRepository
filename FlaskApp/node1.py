...
def mine():
    ...
    return render_template("mine.html", index =index, transactions=json.dumps(transactions), proof=proof, previous_hash=previous_hash)
...
def new_transaction_seller():
    ...
    if request.method == 'POST':
        ...
        sellerA=request.form['sender']
        sellerB=request.form['reciever']
        product_name=request.form['product']
        quantity=request.form['quantity']
        seller_publicKey=request.form['pubkey']
    ...
    #     return 'Missing values', 400
    # Create a new Transaction
    index = blockchain.new_transaction_seller( manufacturer, sellerA,sellerB, product_name, int(quantity),seller_publicKey)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--port', default=9001, type=int, help='port to listen on')
